...
export async function POST(
  ...
  const { data: proposal } = await admin
    .from("proposals")
    .select("slug, client_name, client_email, price, currency")
    .eq("id", id)
    .maybeSingle();
  ...
  const sent = await notifyClient(
    ...
    `<div style="font-family: -apple-system, sans-serif; max-width: 480px; color: #1a1a1a;">
       ...
       <p>We're happy you're considering doing business with us. Please see your proposal below — you can review everything, sign, and pay directly from the page.</p>
